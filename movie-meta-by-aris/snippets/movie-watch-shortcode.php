 ...
 * SEO: sheet column L = SEO Title, column M = Meta Description (applied on /watch/?id=…).
 */
...
/* -------------------------------------------------------------------------
 * SEO Title (col L) + Meta Description (col M) from the same movies sheet
 * ---------------------------------------------------------------------- */
...
if (!function_exists('mmba_snip_fetch_seo_meta_map')) {
     ...
     * IDs match the plugin catalog (same title+link / series seed hashing).
     *
     * @return array<string, array{title: string, description: string}>
     */
...
if (!function_exists('mmba_snip_watch_seo_apply')) {
    /**
     * Register document title / meta description filters for the current watch page.
     *
     * @param string $movie_id Catalog id (m… / s…).
     */
...
if (!function_exists('mmba_snip_watch_seo_boot')) {
    /**
     * Early bootstrap so title/meta apply before wp_head (shortcodes run too late alone).
     */
